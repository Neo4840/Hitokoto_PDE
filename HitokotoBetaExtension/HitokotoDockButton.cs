...
internal sealed partial class HitokotoDockButton : InvokableCommand, INotifyPropChanged
{
    ...
    {
        // 复制句子文本到剪贴板
        if (!string.IsNullOrEmpty(_fullText) && _fullText != "加载一言...")
        {
            ClipboardHelper.SetText(_fullText);
            // 显示一个短暂的 Toast 提示
            var toast = new ToastStatusMessage($"已复制：{_fullText}");
            toast.Show();
        }
    ...
    {
        _fullText = hitokotoText;
        // 截断过长的句子以适应 Dock 栏宽度（约 28 个字符）
